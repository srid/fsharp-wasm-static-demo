...
let elClass el cls children = el [ attr.``class`` cls ] children

let divClass = elClass div
...
type Input() =
    ...
    override this.ShouldRender(oldModel, newModel) = oldModel.value <> newModel.value
...
type UserCard() =
    ...
    override this.View user dispatch =
        let userName =
            user.name |> Option.defaultValue user.login

        let userWebUrl =
            match user.blog with
            | None -> user.html_url
            | Some "" -> user.html_url // In case the user has nullfied this field
            | Some x -> x

        let location =
            user.location |> Option.defaultValue "Earth"

        let mapUrl =
            $"https://www.bing.com/maps/?q={location}"

        elClass
            figure
            "md:flex bg-gray-100 rounded-xl p-8 md:p-0"
            [ img [ attr.``class`` "w-32 h-32 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto"
                    attr.width 284
                    attr.height 512
              ...
              divClass
                  ...
                  [ blockquote [] [
                        b [] [
                            aLink userWebUrl [ text userName ]
                        ]
                        text " lives in "
                        ...
                        br []
                        br []
                        divClass "font-mono text-xs text-gray-400" [ text $"{user}" ]
                    ] ] ]
...
type Message = GitHubUserMessage of RemoteDataMessage<User, string>
...
let update (GitHubUserMessage remoteDataMsg) model =
    ...
    | SetLoadedData i -> { model with userInfo = Loaded i }, Cmd.none
    | SetLoadError e -> { model with userInfo = LoadError e }, Cmd.none
    | LoadData s ->
        { model with
              userName = s.Trim()
              userInfo = Loading },
...
let view model dispatch =
    divClass
        ...
        [ elClass
            h1
            "p-2 m-4 text-center text-4xl font-bold"
            [ aLink "https://github.com/srid/GitHubUser" [ text "GitHubUser" ] ]
          elClass p "" [ text "Enter the Github username you wish to query for and hit ENTER." ]
          divClass
              "border-1 my-1 py-1"
              [ ecomp<Input, _, _>
                    []
          ...
          | Loaded user -> ecomp<UserCard, _, _> [] user (fun _ -> ())
          | LoadError err -> divClass "bg-red-200 overflow-x-auto p-1 text-xs" [ pre [] [ text (err.ToString()) ] ] ]
...
type MyApp() =
    ...
    override this.Program =
        ...
        let init = initialModel, Cmd.ofMsg (GitHubUserMessage(LoadData "srid"))

        Program.mkProgram (fun _ -> init) update view
        |> Program.withTrace
            (fun msg model ->
                this.JSRuntime.InvokeVoidAsync("console.log", msg, model)
                |> ignore)
